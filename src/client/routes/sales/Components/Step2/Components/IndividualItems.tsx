...
import { SalesPresentationItemClassItemNumber, SalesPresentationItemClass } from 'graphql';
...
export interface IIndividualItemsProps {
  ...
  items: Partial<SalesPresentationItemClassItemNumber>[];
...
const IndividualItems: React.FunctionComponent<IIndividualItemsProps> = ({ hasPriceLevel, items, form, removeItem, priceAdjustPercent, priceAdjustPercentTo, cubeAdjust, cubeAdjustTo, priceLevels }) => {
  ...
  useEffect(() => {
    if (priceAdjustPercent !== priceAdjustment || priceAdjustPercentTo !== priceAdjustmentTo || cubeAdjust !== cubeAdjustment || cubeAdjustTo !== cubeAdjustmentTo) {
        ...
        items.map((item, index: number) => {
          priceLevels.map(priceLevel => {
            if (some(adjustablePriceLevels, apl => apl === priceLevel)) {
              const customPriceFieldValue = form.getFieldValue(`items[${index}].${priceLevel}_CustomPricing`);
              if (!customPriceFieldValue) {
                const fieldValue = Number(item[`${priceLevel}_Original`] ? item[`${priceLevel}_Original`]! : 0);
                const fieldName = `items[${index}].${priceLevel}`;

                let adjustedPrice = fieldValue;
                let cubes = form.getFieldValue(`items[${index}].cubes`);
  ...
  }, [priceAdjustPercent, priceAdjustPercentTo, cubeAdjust, cubeAdjustTo, items, form]);
  ...
  const getAdjustedPrice = (priceLevel: string, index: number): number => {
    const fieldValue = Number(items[index][`${priceLevel}_Original`]);
    if (some(adjustablePriceLevels, apl => apl === priceLevel)) {
      let adjustedPrice = fieldValue;
      let cubes = form.getFieldValue(`items[${index}].cubes`);
  ...
  const resetPrice = (priceLevel: string, index: number): void => {
    form.setFieldsValue({ [`items[${index}].${priceLevel}_CustomPricing`]: 0 });
    form.setFieldsValue({ [`items[${index}].${priceLevel}`]: getAdjustedPrice(priceLevel, index) });
  };
  ...
  return (
    <div>
      {/* <Row type="flex" gutter={10} style={{ alignItems: 'center' }}>
        ...
        <Col style={{ flex: 3, fontWeight: 700 }}>Item Description</Col>
        <Col style={{ flex: 1, fontWeight: 700 }}>Qty</Col>
        ...
        {(provided: DroppableProvided, snapshot: DroppableStateSnapshot) => (
          <div ref={provided.innerRef} {...provided.droppableProps} style={getListStyle(snapshot.isDraggingOver)}>
            {items.map((item, index) => (
              <Draggable key={`Drg-${item.itemNumber}|${index}`} draggableId={index.toString()} index={index}>
                {(provided: DraggableProvided, snapshot: DraggableStateSnapshot) => (
                  <div>
                    <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} style={getItemStyle(provided.draggableProps.style, snapshot.isDragging)}>
                      <Row key={`${item.itemNumber}|${index}`} type="flex" gutter={10} style={{ alignItems: 'center' }}>
                        <Col style={{ flex: 2, margin: 0, fontSize: 14 }}>
                          <Button icon="delete" type="danger" size="small" style={{ marginRight: 7 }} onClick={e => removeItem(e, item.itemNumber!)}></Button>
                          {item.itemNumber}
                        </Col>
                        <Col style={{ flex: 3, margin: 0, fontSize: 14 }}>{item.itemDescription}</Col>
                        <Col style={{ flex: 1, margin: 0, fontSize: 14 }}>1</Col>
                        {form.getFieldDecorator(`items[${index}].itemDescription`, {
                          initialValue: item.itemDescription,
                        })(<Input type="hidden" />)}
                        {form.getFieldDecorator(`items[${index}].itemNumber`, {
                          initialValue: item.itemNumber,
                        })(<Input type="hidden" />)}
                        {form.getFieldDecorator(`items[${index}].cubes`, {
                          initialValue: item.cubes,
                        })(<Input type="hidden" />)}
                        {form.getFieldDecorator(`items[${index}].dimensions`, {
                          initialValue: item.dimensions,
                        })(<Input type="hidden" />)}
                        {form.getFieldDecorator(`items[${index}].displayOrder`, {
                          initialValue: item.displayOrder ? item.displayOrder : index,
                        })(<Input type="hidden" />)}
                        {hasPriceLevel('DROPSHIP') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].DROPSHIP_Original`, {
                              initialValue: item[`DROPSHIP_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].DROPSHIP_CustomPricing`, {
                              initialValue: item[`DROPSHIP_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].DROPSHIP`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    form.setFieldsValue({
                                      [`items[${index}].DROPSHIP_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].DROPSHIP_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                        ...
                        {hasPriceLevel('DROPSHIP_M') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].DROPSHIP_M_Original`, {
                              initialValue: item[`DROPSHIP_M_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].DROPSHIP_M_CustomPricing`, {
                              initialValue: item[`DROPSHIP_M_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].DROPSHIP_M`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    e.currentTarget.blur();
                                    form.setFieldsValue({
                                      [`items[${index}].DROPSHIP_M_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].DROPSHIP_M_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                                style={{ fontSize: '24px', position: 'relative', top: '5px', color: 'red', marginLeft: '1px' }}
                        ...
                        {hasPriceLevel('DROPSHIP_X') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].DROPSHIP_X_Original`, {
                              initialValue: item[`DROPSHIP_X_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].DROPSHIP_X_CustomPricing`, {
                              initialValue: item[`DROPSHIP_X_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].DROPSHIP_X`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    e.currentTarget.blur();
                                    form.setFieldsValue({
                                      [`items[${index}].DROPSHIP_X_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].DROPSHIP_X_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                                style={{ fontSize: '24px', position: 'relative', top: '5px', color: 'red', marginLeft: '1px' }}
                        ...
                        {hasPriceLevel('FOB') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].FOB_Original`, {
                              initialValue: item[`FOB_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].FOB_CustomPricing`, {
                              initialValue: item[`FOB_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].FOB`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    e.currentTarget.blur();
                                    form.setFieldsValue({
                                      [`items[${index}].FOB_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].FOB_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                                style={{ fontSize: '24px', position: 'relative', top: '5px', color: 'red', marginLeft: '1px' }}
                        ...
                        {hasPriceLevel('FOB_M') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].FOB_M_Original`, {
                              initialValue: item[`FOB_M_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].FOB_M_CustomPricing`, {
                              initialValue: item[`FOB_M_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].FOB_M`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    e.currentTarget.blur();
                                    form.setFieldsValue({
                                      [`items[${index}].FOB_M_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].FOB_M_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                                style={{ fontSize: '24px', position: 'relative', top: '5px', color: 'red', marginLeft: '1px' }}
                        ...
                        {hasPriceLevel('LEVEL0') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].LEVEL0_Original`, {
                              initialValue: item[`LEVEL0_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].LEVEL0_CustomPricing`, {
                              initialValue: item[`LEVEL0_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].LEVEL0`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    e.currentTarget.blur();
                                    form.setFieldsValue({
                                      [`items[${index}].LEVEL0_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].LEVEL0_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                                style={{ fontSize: '24px', position: 'relative', top: '5px', color: 'red', marginLeft: '1px' }}
                        ...
                        {hasPriceLevel('LEVEL1') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].LEVEL1_Original`, {
                              initialValue: item[`LEVEL1_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].LEVEL1_CustomPricing`, {
                              initialValue: item[`LEVEL1_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].LEVEL1`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    e.currentTarget.blur();
                                    form.setFieldsValue({
                                      [`items[${index}].LEVEL1_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].LEVEL1_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                                style={{ fontSize: '24px', position: 'relative', top: '5px', color: 'red', marginLeft: '1px' }}
                        ...
                        {hasPriceLevel('LEVEL2') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].LEVEL2_Original`, {
                              initialValue: item[`LEVEL2_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].LEVEL2_CustomPricing`, {
                              initialValue: item[`LEVEL2_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].LEVEL2`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    e.currentTarget.blur();
                                    form.setFieldsValue({
                                      [`items[${index}].LEVEL2_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].LEVEL2_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                                style={{ fontSize: '24px', position: 'relative', top: '5px', color: 'red', marginLeft: '1px' }}
                        ...
                        {hasPriceLevel('LEVEL3') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].LEVEL3_Original`, {
                              initialValue: item[`LEVEL3_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].LEVEL3_CustomPricing`, {
                              initialValue: item[`LEVEL3_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].LEVEL3`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    e.currentTarget.blur();
                                    form.setFieldsValue({
                                      [`items[${index}].LEVEL3_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].LEVEL3_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                                style={{ fontSize: '24px', position: 'relative', top: '5px', color: 'red', marginLeft: '1px' }}
                        ...
                        {hasPriceLevel('MIX_FULL') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].MIX_FULL_Original`, {
                              initialValue: item[`MIX_FULL_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].MIX_FULL_CustomPricing`, {
                              initialValue: item[`MIX_FULL_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].MIX_FULL`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    e.currentTarget.blur();
                                    form.setFieldsValue({
                                      [`items[${index}].MIX_FULL_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].MIX_FULL_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                                style={{ fontSize: '24px', position: 'relative', top: '5px', color: 'red', marginLeft: '1px' }}
                        ...
                        {hasPriceLevel('MIX_HALF') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].MIX_HALF_Original`, {
                              initialValue: item[`MIX_HALF_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].MIX_HALF_CustomPricing`, {
                              initialValue: item[`MIX_HALF_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].MIX_HALF`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    e.currentTarget.blur();
                                    form.setFieldsValue({
                                      [`items[${index}].MIX_HALF_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].MIX_HALF_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                                style={{ fontSize: '24px', position: 'relative', top: '5px', color: 'red', marginLeft: '1px' }}
                        ...
                        {hasPriceLevel('MIX_QTR') && (
                          <Col style={{ flex: 1, display: 'flex' }}>
                            {form.getFieldDecorator(`items[${index}].MIX_QTR_Original`, {
                              initialValue: item[`MIX_QTR_Original`],
                            })(<Input type="hidden" />)}
                            {form.getFieldDecorator(`items[${index}].MIX_QTR_CustomPricing`, {
                              initialValue: item[`MIX_QTR_CustomPricing`],
                            })(<Input type="hidden" />)}
                            <FormItem hasFeedback style={{ margin: '5px 0', flex: 1 }}>
                              {form.getFieldDecorator(`items[${index}].MIX_QTR`, {
                                ...
                                <InputNumber
                                  ...
                                  onBlur={e => {
                                    ...
                                    e.currentTarget.blur();
                                    form.setFieldsValue({
                                      [`items[${index}].MIX_QTR_CustomPricing`]: 1,
                                    });
                                  }}
                                  onPressEnter={e => {
                                    e.preventDefault();
                            ...
                            {form.getFieldValue(`items[${index}].MIX_QTR_CustomPricing`) > 0 && (
                              <Icon
                                type="close-square"
                                theme="filled"
                                style={{ fontSize: '24px', position: 'relative', top: '5px', color: 'red', marginLeft: '1px' }}
